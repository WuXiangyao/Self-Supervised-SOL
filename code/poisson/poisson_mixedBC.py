...
This is the code for mixed BC experiment in Section 4.3.3, and we swapped x and y in the code for simplicity 
"""
...
#### data settings
data_label = "res401alpha2mixed"
data_name = "elli2d-Nx401-GRF-alpha2-mixedBC.pt"
...
u0 = kx * torch.sin(torch.pi * X) * torch.cosh(torch.pi - torch.pi * Y)
u0 = u0.reshape(Nx, Nx).to(device)

## model
model = (
    OPNO2d(modes, modes, width, bandwidth, bandwidth, skip=sol_skipflag)
    .to(device)
    .double()
)
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        if train_withdata:
            ...
            weak_loss = weak_mix(out, f, u0)
            strong_loss = strong_mix(out, f, u0)
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            out = model(x).reshape(b, Nx, Nx) + u0
