...
def savefig(name='temp', format='png', dpi=600):
    pic_file = pic_path + name + '.' + format
    plt.savefig(pic_file, bbox_inches='tight', pad_inches=0.2, dpi=dpi)
    print('save as ', pic_file)
...
class my_plt1d(object):
    ...
    def ppt(self, model, x, y):
        ...
        plt.scatter(self.mesh, yy, color=self.clr, s=400, alpha=0.50, label=self.label)
